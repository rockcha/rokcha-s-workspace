...
폴더별 보기를 기본으로 하며 폴더를 펼쳐 메모와 하위 폴더를 확인합니다. 폴더 추가 시 위치를 최상위 또는 기존 폴더로 지정합니다. 필터·메모 저장·이동에는 전체 폴더 경로를 표시합니다. 최신순은 작성일 기준입니다. 메모 행을 누르면 제목·내용과 폴더를 수정할 수 있고 이동 아이콘으로 폴더만 변경할 수도 있습니다. 폴더·메모 편집에는 로컬 shadcn `Dialog`, 삭제에는 `AlertDialog`를 사용합니다. 폴더 삭제는 모든 하위 폴더와 포함된 메모 수, 복구 불가 안내를 표시합니다.

대화상자는 키보드 포커스를 내부에 유지하고 닫을 때 실행 버튼으로 돌려줍니다. 삭제로 버튼이 사라진 경우 폴더 추가 버튼으로 돌려줍니다. 저장 중에는 재제출과 닫기를 막고, 실패 시 입력과 대화상자를 유지합니다. 성공 알림은 기존 Toaster에 고정 ID로 전달합니다. `npm run test:ui`에서 이 흐름과 320px 모바일 배치를 검사합니다.
